...
class XMODEM(object):
    ...
    def __init__(self, mode='ymodem', pad=b'\x1a'):
        self.mode = mode
        self.pad = pad
    ...
    def getc(self, size, timeout=10):
        machine.stdin_take()
        res = machine.stdin_get(size,timeout*1000)
        machine.stdin_give()
        if res:
            return res.encode()
        else:
    ...
    def putc(self, data, timeout=1):
        return machine.stdout_put(data.decode())  # note that this ignores the timeout
        ...
        while True:
            ...
            if char:
                if char == NAK:
                    ...
                    pass
                    # self.log.error('send error: expected NAK, CRC, or CAN; got %r', char)
    ...
    def _verify_recv_checksum(self, crc_mode, data):
        if crc_mode:
            ...
            if not valid:
                pass
                # self.log.warning('recv error: checksum fail (theirs=%04x, ours=%04x), ', their_sum, our_sum)
        else:
            ...
            if not valid:
                pass
                # self.log.warning('recv error: checksum fail (theirs=%02x, ours=%02x)', their_sum, our_sum)
        return valid, data
